      ...
      <div class="vx-col sm:w-full">
        <vs-card hover="true">
          ...
          <div class="vx-row mb-6">
            ...
            <div class="vx-col sm:w-2/3 w-full">
              <tree-select  v-model="category.parent_id" :options="categories" name="categories" :normalizer="normalizer"/>
              <span class="text-danger text-sm" v-show="errors.has(`categories`)">{{ errors.first(`categories`) }}</span>

            </div>
          </div>
          <div slot="footer">
            <vs-button color="success" :disabled="disable" type="filled" @click="saveCategory">ذخیره</vs-button>
          </div>
        </vs-card>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      '$store.state.articleCategory.errors': {
        deep: false,
        handler(errors) {
    ...
    methods: {
      ...
      saveCategory() {
        ...
        this.$validator.validateAll().then(validated => {
          if (validated) {
            this.$store.commit('articleCategory/SET_CATEGORY', this.category)
