    ...
    hovered (val) {
      this.$nextTick(() => {
        if (val) {
          ...
          if (windowInnerHeight - dd.getBoundingClientRect().top - dd.getBoundingClientRect().height - 28 < 1) {
            const maxHeight = windowInnerHeight - dd.getBoundingClientRect().top - 70
            dd.style.maxHeight = `${maxHeight}px`
            dd.style.overflowY = 'auto'
            dd.style.overflowX = 'hidden'
          }

          if (dd.getBoundingClientRect().left + dd.offsetWidth - (windowInnerWidth - 16) >= 0 || this.$parent.dropLeft) {
            this.dropLeft = true
          }
