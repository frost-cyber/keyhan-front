...
name: "advice",
  ...
  methods:{
    getAdvices(){
      this.$store.dispatch('settings/getAdvices').then(res => {
        if (res.status === 200) {
          this.advices = res.data
          this.$store.commit('settings/SET_ADVICE',this.$cloneObject(res.data))
        }

      })
    },
    deleteAdvice(advice){
      this.$store.dispatch('settings/deleteAdvice', advice).then((res) =>{
        if (res.status === 200){
          this.$store.commit('settings/DELETE_ADVICE', advice)
          this.$vs.notify({
    ...
    changeChecked(id){
      this.$store.dispatch('settings/toggleCheck',id)
