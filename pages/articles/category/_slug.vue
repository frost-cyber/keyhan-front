        ...
        <div class="grid grid-cols-12 gap-5">
          ...
          <div class="left-box col-span-12 md:col-span-8 lg:col-span-9 p-3 border-cool-100 border rounded-lg">
            <div class=" grid grid-cols-12 gap-5 md:gap-4">
              <article v-for="(article , index ) in articles" :key="index"   class="col-span-6 lg:col-span-4" :class="'md:col-span-'+article.responsivMd +' lg:col-span-'+article.responsivLg">
                <div class="img-product mb-4">
                  <nuxt-link class="img-thum" :to="{name:'articles-slug' , params:{slug:article.slug}}">
                    <img class="rounded-lg  mx-auto"  :src="article.thumbnail.link || require('@/assets/images/portrait/small/avatar-s-20.jpg')" >
                  </nuxt-link>
                </div>
...
export default {
  ...
  fetch() {
    this.$store.dispatch('articleCategory/getCategories' , {children : this.$route.params.slug})
    this.getArticles()
  },
  ...
  computed:{
    categories(){
      return this.$store.getters['articleCategory/getCategories']
    }
  },
  methods: {
    getArticles() {
